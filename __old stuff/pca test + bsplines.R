...
###########
#START HERE!!
mydir <- "/Users/aaronfisher/Documents/JH/Biostatistics V & VI/Bst VI Final Project with Chen"
setwd(mydir)
...
ups<-pcx$load #Matrix of eigen vectors (by column) denoted ψ
image(ups)
image(t(ups)%*%ups)
...
for(kx in 1:K_x){
	for (kb in 1:K_b){
		J[kx,kb]<-sum(ups[,kx] * bsplines[,kb]) #*1/p to integrate from 0 to 1
	}	
}
C<-xScoreKx
...
#this code still has some errors in
...
writeDatafileR(DATA=dat4winbugs,towhere='Dat4Winbugs.txt')
