## This is the main function for calculating the principal surface of a given 2D dataset

PrinCurve <- function(dat, sub.size=100, grid.n=100, max.iter=20, err.tol=0.05, neighbor=0.03, kn=10, initial.para='default'){
  set.seed(0)
  ...
  if (is.character(initial.para)){
    proj <- score1
  } else {
    proj <- initial.para[samp]
  }
  j <- 1
  diff <- 1
  library(mgcv)
